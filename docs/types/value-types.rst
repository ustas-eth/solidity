...
.. warning::
    Comparison of internal function pointers can have unexpected results with the legacy optimizer enabled,
    as it can collapse identical functions into one, which will then lead to said functions comparing as equal instead of not.
    Such comparisons are not advised, and will lead to the compiler issuing a warning, until the next breaking release (0.9.0),
    where the warning will be upgraded to an error, thereby making such comparisons disallowed.
